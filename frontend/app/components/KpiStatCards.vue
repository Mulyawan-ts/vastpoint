...
const avgWaterLevel = computed(() => {
  ...
  const total = props.telemetry.reduce(
    (acc, curr) => acc + curr.water_level,
    0,
  );
...
<template>
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    <div
      class="bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 p-4 rounded-xl shadow-sm flex items-center justify-between"
    >
      <div>
        <p class="text-xs text-sub font-semibold uppercase tracking-wider">
          Active Sensors
        </p>
        <h3
          class="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1 m-0"
        >
          {{ totalSensors }}
        </h3>
        <span
      ...
      <div
        class="p-3 bg-sky-100 dark:bg-sky-950/80 text-sky-600 dark:text-sky-400 rounded-lg"
      >
    ...
    >
      <div>
        ...
        <h3
          class="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1 m-0"
        >
          {{ avgWaterLevel }} <span class="text-xs font-normal">m</span>
        </h3>
        <span class="text-[10px] text-slate-400 font-medium"
          >Real-time aggregate</span
        >
      </div>
      <div
        class="p-3 bg-blue-100 dark:bg-blue-950/80 text-blue-600 dark:text-blue-400 rounded-lg"
      >
    ...
    >
      <div>
        ...
        <h3
          class="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1 m-0"
        >
          {{ maxWaterLevel }} <span class="text-xs font-normal">m</span>
        </h3>
        <span class="text-[10px] text-amber-500 font-medium"
          >Highest detected</span
        >
      </div>
      <div
        class="p-3 bg-amber-100 dark:bg-amber-950/80 text-amber-600 dark:text-amber-400 rounded-lg"
      >
    ...
    >
      <div>
        ...
        <h3
          class="text-2xl font-bold mt-1 m-0"
          :class="
            activeAlertsCount > 0
        ...
        <span
          :class="
            activeAlertsCount > 0 ? 'text-rose-500 font-bold' : 'text-slate-400'
          "
          class="text-[10px]"
        >
