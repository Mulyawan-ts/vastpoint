...
<template>
  <section class="card-panel">
    <div class="flex flex-wrap justify-between items-center mb-4 gap-2">
      <h2
        class="m-0 text-lg text-sky-600 dark:text-sky-400 flex items-center gap-2"
      >
      ...
      <div
        class="flex gap-1 bg-slate-200 dark:bg-slate-900 p-1 rounded-lg text-xs font-medium"
      >
        <button
          v-for="r in ranges"
          :key="r.value"
        ...
        <div class="flex justify-between text-xs text-sub">
          <span
            ...
            <b class="text-slate-700 dark:text-slate-200"
              >{{ metric.avg_water_level }}m</b
            ></span
          >
          <span
            >Max:
            <b class="text-slate-700 dark:text-slate-200"
              >{{ metric.max_water_level }}m</b
            ></span
          >
          <span
            >Records:
